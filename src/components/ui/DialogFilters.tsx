      ...
      <DialogContent>
        ...
        <div>
          <div className='pb-2 grid grid-cols-3 gap-2'>
            ...
            <Button onClick={onAccept}>Применить</Button>
          </div>
          <div className='grid grid-cols-3 grid-rows-3 gap-2 pb-2'>
            {kernel.map((value, index) => <Input key={index} value={value} type='number' onChange={(e) => setKernel((prev) => {
          ...
          {isPreview && (
            <canvas className='max-w-60 max-h-60 mx-auto' width={img.width} height={img.height} ref={canvasRef}/>
          )}
        </div>
        <DialogFooter>
          <Button onClick={onInitImageData}>Сбросить</Button>
          <Button type="submit" onClick={onAcceptToMain}>Подтвердить</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
