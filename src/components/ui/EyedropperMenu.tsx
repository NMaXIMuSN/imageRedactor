...
export const EyedropperMenu = () => {
  ...
  const { firstHistory, secondHistory, resetFirstHistory, resetSecondHistory } = useContext(EyedropperContext)
    ...
    <div className="w-[400px] py-2 px-4 overflow-x-scroll space-y-4">
      <div className="space-y-2">
        <div className="flex justify-between">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="ml-2 cursor-pointer" onClick={() => resetFirstHistory?.()}>
              <ResetIcon width={20} height={20}/>
            </div>
          </div>
          </div>
          <div>
            <div className="-m-2 p-2 cursor-pointer" onClick={() => setCurrentTool?.('')}>
              X
            </div>
          </div>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center">
            <div>
              Вторые цвета <ShortCut className='inline'>Shift+ЛКМ</ShortCut>
            </div>
            <div className="ml-2 cursor-pointer" onClick={() => resetSecondHistory?.()}>
              <ResetIcon width={20} height={20}/>
            </div>
          </div>
        <div className="flex flex-wrap gap-2">
      ...
      </div> : <div>Выберите два цвета для проверки контрастности</div>}
    </div>
  )
}