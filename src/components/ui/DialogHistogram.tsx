...
export const  DialogHistogram = () => {
  ...
  const getLUT = () => {
    ...
    for (let i = 0; i < 256; i++) {
        if (i <= firstX) {
            ...
            lut[i] = firstY + (i - firstX) * (secondY - firstY) / (secondX - firstX);
        }
    }
    return lut;
  }
  ...
  const applyLUT = (imageData: ImageData, lut: Uint8ClampedArray) => {
    const newImageData = new ImageData(imageData.width, imageData.height, { colorSpace: 'srgb'});
  ...
  useEffect(() => {
    if (imgDataToPreview && isPreview) {
      prewCanvasRef.current?.getContext('2d')?.putImageData(imgDataToPreview, 0, 0);
    }
      ...
      <DialogContent className='flex flex-col max-h-[calc(100vh_-_30px)]'>
        ...
        <div className='flex-1 overflow-auto'>
          ...
          <div className='flex flex-col gap-2 items-center'>
            ...
            <Button className='flex-1' onClick={applyCorrection}>Применить</Button>
            </div>
            <div>
              <Label className='flex gap-2 items-center'>
                <Input type='checkbox' checked={isPreview} className='w-4 h-4' onChange={(e) => setIsPreview(e.target.checked)}/>
        ...
        <DialogFooter>
          <Button type="submit" onClick={applyFilter}>Подтвердить</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
